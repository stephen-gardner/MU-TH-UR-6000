...
public class MotherConfig {
  ...
  public void initDefaults() {
    ...
    lang.put("LOG_TIMESTAMP_FMT", "[yyyy-MM-dd HH:mm:ss]");
  ...
  public boolean isValid() {
    ...
    MotherConfig defaultConfig = new MotherConfig();

    defaultConfig.initDefaults();

    for (String key : defaultConfig.getAllMsg().keySet()) {
      if (!lang.containsKey(key) || lang.get(key).isEmpty()) {
        System.err.println(String.format("'%s' lang configuration missing/invalid", key));
        return false;
      }
    }
